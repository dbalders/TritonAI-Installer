...
- Managed plugin source: stable release packaging resolves the highest canonical `vMAJOR.MINOR.PATCH` tag once, freezes its full commit, and selects the sorted production allowlist `google-workspace,microsoft-365`. Complete explicit pins override this for exact rebuilds. It never follows a moving branch or infers the nearby development checkout. Selected package contents are validated and atomically staged as a Harness build input.

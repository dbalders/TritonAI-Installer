...
4. Wait for both builds and the draft job. Signing environments restrict releases to `main` without requiring per-build approval. Mac verifies Developer ID signing, notarization, stapling, Gatekeeper and native packaged boot. Windows verifies publisher, trusted timestamp and Setup/portable native packaged boot. Both compare every consumed Harness input against artifacts from the selected run and enforce the bundled Codex policy and plugin composition.

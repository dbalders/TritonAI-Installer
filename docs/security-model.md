...
- Accept managed plugin code only through the canonical `dbalders/TritonAI-Plugins` repository. Stable packaging uses the reviewed Installer catalog and verifies its exact source, package and manifest digests, and SDK artifact descriptor digest. The production composition is globally stable and required; unenforced per-package policy metadata is rejected. Explicit ref/commit pins remain development inputs and cannot override production selection.
- Require the bundled Harness release to attest that exact plugin composition and bind the attestation to the exact DMG/EXE bytes. Raw plugin directories are never copied separately into the installed machine; Harness permits only the reviewed, digest-bound factories in its build and has no runtime discovery or unreviewed package-loading path.
...
- On Windows, bind every packaged Harness payload to an explicit, immutable `authenticode` or `unsigned` trust policy. Both modes require the exact release version, filename, size, SHA-512, and plugin-composition proof. The current unsigned release lane skips only publisher checks, warns the operator and user, and may trigger SmartScreen. Once UCSD signing exists, the signed lane additionally requires native Authenticode verification against publisher `University of California San Diego`, a signer certificate, and a trusted timestamp before vendoring, before NSIS execution, and on the installed Harness executable. Missing or mismatched policies fail closed; ambient runtime environment variables cannot downgrade a signed package.
...
- Route TritonAI Harness through Codex by setting the provider instance to `codex`, passing `UCSD_AI_BASE_URL` and `TRITONAI_API_KEY`, exposing TritonAI-routed Codex models, defaulting to `api-glm-5.3`, and disabling stale non-Codex provider entries.
...
- Avoid relying on Homebrew, system Node, global machine package state, or a live Node.js download during packaged setup. Release preparation requires the official Node checksum and archive to match a repository-reviewed per-platform SHA-256, then binds the filename, size, version, target, and digest in the packaged manifest; runtime installation re-verifies that manifest and archive before a transactional directory swap.
- Treat native launchability as part of artifact identity. Stable macOS packaging boots the final mounted candidate. Unsigned Windows packaging records hashes and validates PE structure on macOS/Wine or Windows, while the separate native-Windows verifier re-hashes and boots the exact portable and Setup candidates before writing runtime proof. The combined signed release contract rejects boot proofs whose SHA-256, version, platform, architecture, packaged flag, or health duration does not match.
...
- Keep the pinned Electron/Electron Builder toolchain inside the package-age, registry-signature, attestation, and zero-high-vulnerability gates when advancing it.
- Make telemetry/log upload opt-in or UCSD-policy-driven, with clear redaction rules.
- Move remaining development-only live-download fallbacks behind a UCSD-maintained, pre-vetted artifact mirror.

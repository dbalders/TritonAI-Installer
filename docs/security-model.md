...
- Accept managed plugin code only through the canonical `dbalders/TritonAI-Plugins` repository. Stable packaging uses the reviewed Installer catalog and verifies its exact source, publisher, package and manifest digests, SDK artifact descriptor digest, release channel, required status, and retirement policy. Explicit ref/commit pins remain development inputs and cannot override production policy.

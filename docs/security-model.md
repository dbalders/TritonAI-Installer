...
- Avoid relying on Homebrew, system Node, global machine package state, or a live Node.js download during packaged setup. Release preparation verifies the official Node archive SHA-256 and binds its filename, size, version, and target in the packaged manifest; runtime installation re-verifies that manifest and archive before a transactional directory swap.

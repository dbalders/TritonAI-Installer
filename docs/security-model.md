...
- On Windows, bind every packaged Harness payload to an explicit, immutable `authenticode` or `unsigned` trust policy. Both modes require the exact release version, filename, size, SHA-512, and plugin-composition proof. The current unsigned release lane skips only publisher checks, warns the operator and user, and may trigger SmartScreen. Once UCSD signing exists, the signed lane additionally requires native Authenticode verification against publisher `The Regents of the University of California`, a signer certificate, and a trusted timestamp before vendoring, before NSIS execution, and on the installed Harness executable. Missing or mismatched policies fail closed; ambient runtime environment variables cannot downgrade a signed package.

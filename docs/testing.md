...
- Regressions where stable plugin packaging drifts from the exact sorted `github,google-workspace,microsoft-365,n8n` production selection or accepts a noncanonical origin, moving or mismatched ref/commit, implicit dirty checkout, ambiguous package selection, source/tests, symlinks, special files, unsafe paths, or manifest/package/file drift.
...
- A current managed Codex binary that reports `0.146.0` and should not reinstall.

...
const installChecklist = document.getElementById("install-checklist");
...
function updateStepRail(panelName = state.installPhase === "idle" ? "credentials" : "install") {
  ...
  document.querySelectorAll(".step-rail li").forEach((step) => {
...
document.addEventListener("click", (event) => {
  ...
  if (event.target.closest(".key-info-wrap")) return;
...
result.addEventListener("click", async (event) => {
  ...
  if (action === "copy-support-report") {
    await copySupportReport(event.target);
  }
  if (action === "show-logs") {
    await showLogs(event.target);
  }
});
...
function createPreviewInstallerApi() {
  const listeners = [];
  const preview = {
    id: "t3code",

...
async function installOptionalDesktopApp({ desktopInstaller, paths, platform, arch, emit, env, apiKey, resourcesPath, appRoot }) {
  try {
    return await desktopInstaller({ paths, platform, arch, emit, env, apiKey, resourcesPath, appRoot });
  } catch (error) {
    if (platform === "win32") {

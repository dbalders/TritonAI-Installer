...
export T3CODE_HOME="${paths.t3Home}"
...
function copyMacLauncherIcon(managedAppPath, resourcesDir, emit = () => {}) {
...
$env:T3CODE_HOME = '${escapePowerShellSingleQuoted(paths.t3Home)}'
...
function getBundledMacDmg(options = {}) {
...
function getBundledWindowsInstaller(options = {}) {
...
function findWindowsT3CodeApp(homeDir = os.homedir(), options = {}) {
  ...
  const expectedExecutableNames = mergeUnique(options.expectedExecutableNames || [], WIN_EXE_NAMES);
  const appDirNames = mergeUnique(
    expectedExecutableNames.map((fileName) => path.basename(fileName, ".exe")),
    WIN_INSTALL_DIR_NAMES
  );
  ...
  const candidates = mergeUnique(
    candidateRoots.flatMap((root) => appDirNames.map((dirName) => path.join(root, dirName))),
    candidateRoots
  );

  for (const dir of candidates) {
    const appPath = expectedExecutableNames.map((fileName) => findFile(dir, fileName, 3)).find(Boolean)
      || findLikelyWindowsT3Executable(dir, 3);
...
async function waitForWindowsT3CodeApp(homeDir, options = {}) {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const appPath = findWindowsT3CodeApp(homeDir, options);
    if (appPath) return appPath;
...
function findLikelyWindowsT3Executable(root, maxDepth) {
  return findFileByPredicate(root, (fileName) => /^tritonai harness(?:\s+\((alpha|beta|preview|nightly|stable)\))?\.exe$/i.test(fileName), maxDepth);
}
...
function windowsExecutableNamesForInstaller(installerPath) {
  const parsed = parseWindowsInstallerMetadata(installerPath);
  return mergeUnique([
    parsed.productName && `${parsed.productName}.exe`,
    parsed.fileDescription && `${parsed.fileDescription.replace(/\s+desktop build$/i, "").trim()}.exe`
  ], WIN_EXE_NAMES).filter(Boolean);
}

function parseWindowsInstallerMetadata(installerPath) {
  try {
    const data = fs.readFileSync(installerPath);
    const text = data.toString("utf16le", 0, Math.min(data.length, 96 * 1024));
    return {
      productName: readWindowsVersionValue(text, "ProductName"),
      fileDescription: readWindowsVersionValue(text, "FileDescription")
    };
  } catch (_error) {
    return {};
  }
...
function readWindowsVersionValue(text, key) {
...
function run(command, args, emit, options = {}) {
  return new Promise((resolve, reject) => {
...
function runCapture(command, args, emit, options = {}) {
  return new Promise((resolve, reject) => {
    emit(`$ ${command} ${args.join(" ")}`);
    let stdout = "";
    let stderr = "";
...
function runPowerShell(script, emit, options = {}) {
  return run("powershell.exe", powerShellArgs(script), emit, { ...options, shell: false });
}

function runPowerShellCapture(script, emit, options = {}) {

...
app.whenReady().then(() => {
  ...
  ipcMain.handle("installer:open-docs", async (_event, url) => {
    if (!url) {
  ...
  ipcMain.handle("installer:start", async (event, payload) => {
  ...
  ipcMain.handle("installer:finish", async (event, payload = {}) => {
...
function getLaunchTarget(toolId, desktopApps, homeDir) {
  if (toolId === "t3code") {
...
function scheduleVolumeEject(volumePath) {

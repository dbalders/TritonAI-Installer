...
async function assertExistingApiKeyLookup() {
  assert.strictEqual(
    readApiKeyFromEnvText("export TRITONAI_API_KEY='existing-key'"),
    "existing-key"
  );
  assert.strictEqual(
  ...
  try {
    ...
    assert.deepStrictEqual(
      await findExistingApiKey({
        homeDir: tempRoot,
        platform: "darwin",
        env: { TRITONAI_API_KEY: " process-key " }
      }),
      { apiKey: "process-key", source: "processEnvironment" }
    );

    const winPaths = getPaths(tempRoot, "win32");
    fs.writeFileSync(winPaths.envFile, '$env:TRITONAI_API_KEY = "win-file-key"\n');
    assert.deepStrictEqual(
      await findExistingApiKey({
        homeDir: tempRoot,
        platform: "win32",
    ...
    assert.deepStrictEqual(
      ...
      { apiKey: "win-file-key", source: "installerEnvFile" }
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
  }
...
async function assertRunInstallStopsBeforeDesktopWhenConnectionFails() {
  ...
  try {
    ...
    try {
      await runInstall({ apiKey: "test-api-key" }, {
        homeDir: tempRoot,
        platform: "darwin",
        arch: runtimeArch,
        emit: () => {},
...
async function runDryRun(platform, options) {
  ...
  try {
    ...
    assert(!fs.readFileSync(installResult.diagnostics.logFile, "utf8").includes("test-api-key"), "installer log should redact the access key");
    assert(!JSON.stringify(supportReport).includes("test-api-key"), "support report should redact the access key");
    ...
    assert.strictEqual(t3Settings.textGenerationModelSelection.model, UCSD.codexModel);
    ...
    assert.strictEqual(t3DevSettings.textGenerationModelSelection.model, UCSD.codexModel);
    ...
    assert(t3DefaultsPatcher.includes(UCSD.codexModel));
    ...
    assert(!t3DefaultsPatcher.includes("test-api-key"), "defaults patcher should not embed the access key");
    ...
    assert.strictEqual(connectionChecks[0].apiKey, "test-api-key");
    ...
    assert.strictEqual(t3CodeDesktopInstalls[0].env.TRITONAI_API_KEY, "test-api-key");
...
async function assertOnboardingWorkspaceOnlySeedsOnFirstInstall() {
  ...
  try {
    ...
    await runInstall({ apiKey: "test-api-key" }, runtime);
